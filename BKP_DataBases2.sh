...
function func_ExportDB() {
	...
	if [[ $? == 0 ]]; then
		...
		if [[ $sepFiles == 0 ]]; then
			...
			# Pasta das tabelas
			tablesFolder="$folderTemporary/tables"
			...
			/usr/bin/mysqldump -h $expIP -u $expUser -p$expPass --routines --triggers --single-transaction --quick --no-data $expDB >$folderTemporary/$expPrefix"_"$expDB"_"$current_datetime.sql
...
function func_CompactDB() {
	...
	if [[ $compactType == "" ]]; then
		...
		for fileExp in $(ls $folderTemporary/*"_"$current_datetime.sql); do
